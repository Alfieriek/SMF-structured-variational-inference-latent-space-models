...
mean_field_DN_adaptive = function(Y,mean_beta_prior, sigma_beta_prior, d=2, rho=1, gap =50,max_iter=100,alpha=0.95){
  ...
  while(k<K && ind ==0){
    ...
    if( train_auc[k]-train_auc[k-1]< gap || k>max_iter){
      ind = 1
    }    else{
