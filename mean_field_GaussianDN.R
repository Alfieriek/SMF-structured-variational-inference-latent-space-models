...
mean_field_GaussainDN = function(Y,trans_sd ,mean_beta_prior, sigma_beta_prior,rho, sigma_X1, sigma , gap =1,max_iter=100, alpha=0.95){
  ...
  while(k<K && ind ==0){
    ...
    for (i in inde[1:n]){
      ...
      #       pred_old[r] = mean_beta_new+t(Mean_X[[t]][i,])%*% Mean_X[[t]][j,]
      #       res[r] = Y[[t]][i,j]
      #       r=r+1
      #     }
      #   }
      #   if (sum(abs(pred_mean-res))>sum(abs(pred_old-res))){Mean_X_new[[t]][i,] =Mean_X[[t]][i,] }
       }
    ...
    # for (t in 1:T){
    #   for (i in 1:n){
    #     pred_err[r] = sum((Mean_X_new[[t]][i,]-X[[t]][i,])^2)
    #     r=r+1
    #   }
    # }
    # 
    # norm_stop[k]=sqrt(sum((pred_err)/T/n))
    
    

    
    if(err[k]-err[k-1] >-0.001 || k>50){
      ind = 1
    }    else{
  ...
  return(list(err=err[2:(k-1)], Mean_X= Mean_X, Sigma_X= Sigma_X,  mean_beta  = mean_beta, sigma_beta =sigma_beta , iter = k-1 ))
}
