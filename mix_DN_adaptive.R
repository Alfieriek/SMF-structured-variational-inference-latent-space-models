...
mix_DN_adaptive = function(Y,mean_beta_prior, d=2, sigma_beta_prior, global_prior='Cauthy',  gap = 1e-2 ,min_iter=1 ,rho=1,alpha=0.95, trans_sd_init =0.1, beta_zero = FALSE){
  ...
  for (t in 1:T){
    ...
    for (i in 1:n){
      Mean_X[[t]][i,] =  0.5*rmvnorm(n=1,mean=rep(0,d), sigma=diag(d)) # randomly initialization for mu_{it}
  ...
  #   mean_beta_new = 0
    ...
    if (global_prior =='Gamma'){
      ...
      tau_inv_sq = exp(besselK.nuAsym(x=sqrt(b), nu=-gig_deg-1,k.max=5,log=T)-besselK.nuAsym(x=sqrt(b), nu=-gig_deg,k.max=5,log=T))/sqrt(b)-2*gig_deg/b
    ...
    for (t in 1:T){
      ...
      auc_res = rep((n-1)*n,0)
      auc_past = rep((n-1)*n,0)
      r=1
      for (i in 1:n){
        for (j in 1:n){
          if (j<i){
            auc_mean[r] = 1/(1+exp(-mean_beta_new-t( Mean_X_new[[t]][i,])%*% Mean_X_new[[t]][j,]))
            pred_mean[r0] = auc_mean[r] 
            auc_res[r] = Y[[t]][i,j]
            auc_past[r] = 1/(1+exp(-mean_beta-t( Mean_X[[t]][i,])%*% Mean_X[[t]][j,]))
            res[r0] = auc_past[r]
            r0=r0+1
            r=r+1
          }
