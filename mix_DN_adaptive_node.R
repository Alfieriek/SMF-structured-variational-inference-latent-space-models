...
mix_DN_adaptive_node = function(Y,mean_beta_prior, sigma_beta_prior,  gap = 0.01 ,min_iter=1 ,rho=1,alpha=0.95){
  ...
  while(k<K && ind ==0){
    ...
    for (t in 1:T){
      for( i in 1:n){
        for (j in 1:n){
          ...
          V_i = Sigma_X[[t]][[i]]
          V_j = Sigma_X[[t]][[j]]
          c = sqrt(distance_squared_inner_prod(M_i,M_j,V_i,V_j) +mean_beta*t(M_i)%*%M_j+mean_beta^2+sigma_beta^2)
          A_Xi[[t]][i,j] = 1/(2*c)*(exp(c)-1)/(exp(c)+1)/(-2)
          if (j!= i){ 
    ...
    for (i in 1:n){
      ...
      Forward.var.new[[1]][[i]] = -rho^(-2)*trans_sd[i]^(4)*(sigma_X1[i]^(-2)*diag(d)+rho^(2)*trans_sd[i]^(-2)*diag(d)+V_X_cumulative[[1]][[i]])
      
      Forward.Mean.new[[1]][i,] = -rho^(-1)*trans_sd[i]^2*
        (M_X_cumulative[[1]][i,])
      
      
      
      Backward.var.new[[T]][[i]] = -rho^(-2)*trans_sd[i]^(4)*(trans_sd[i]^(-2)*diag(d)+V_X_cumulative[[T]][[i]])
      
      Backward.Mean.new[[T]][i,] = -rho^(-1)*trans_sd[i]^2*
        (M_X_cumulative[[T]][i,])
      ...
      if (T>2){
        ...
        for (t2 in 2:(T-1)){
          ...
          Forward.var.new[[t2]][[i]] = -rho^(-2)*trans_sd[i]^(4)*(ginv(Forward.var.new[[t2-1]][[i]])+(1+rho^2)*trans_sd[i]^(-2)*diag(d)+V_X_cumulative[[t2]][[i]])
          
          Forward.Mean.new[[t2]][i,] = -rho^(-1)*trans_sd[i]^2*
            (ginv(Forward.var.new[[t2-1]][[i]])%*%  Forward.Mean.new[[t2-1]][i,]  +M_X_cumulative[[t2]][i,])
          
          
          Backward.var.new[[T+1-t2]][[i]] = -rho^(-2)*trans_sd[i]^(4)*(ginv(Backward.var.new[[T+2-t2]][[i]])+(1+rho^2)*trans_sd[i]^(-2)*diag(d)+V_X_cumulative[[T+1-t2]][[i]])
          
          Backward.Mean.new[[T+1-t2]][i,] = -rho^(-1)*trans_sd[i]^2*
      ...
      for(t in 1:T){
        if(t == 1)
        { Sigma_X_new[[t]][[i]] = ginv(sigma_X1[i]^(-2)*diag(d)+rho^2*trans_sd[i]^(-2)*diag(d)+ginv(Backward.var.new[[t+1]][[i]])+V_X_cumulative[[t]][[i]])
        ...
        if(t==T)
        { Sigma_X_new[[t]][[i]] = ginv(trans_sd[i]^(-2)*diag(d)+ginv(Forward.var.new[[t-1]][[i]])+V_X_cumulative[[t]][[i]])
        ...
        if(1<t  && t<T)
        { Sigma_X_new[[t]][[i]] = ginv((1+rho^2)*trans_sd[i]^(-2)*diag(d)+ginv(Forward.var.new[[t-1]][[i]])+ginv(Backward.var.new[[t+1]][[i]])+V_X_cumulative[[t]][[i]])
      ...
      Cross_cov[[1]][[i]][1:d,1:d] = sigma_X1[i]^(-2)*diag(d)+trans_sd[i]^(-2)*diag(d)+V_X_cumulative[[1]][[i]]
      Cross_cov[[1]][[i]][1:d,(d+1):(2*d)] = -(trans_sd[i]^(-2))*diag(d)
      Cross_cov[[1]][[i]][(d+1):(2*d),1:d] = -(trans_sd[i]^(-2))*diag(d)
      Cross_cov[[1]][[i]][(d+1):(2*d),(d+1):(2*d)] = (trans_sd[i]^(-2))*diag(d)+ginv(Backward.var.new[[3]][[i]])+(trans_sd[i]^(-2))*diag(d)+V_X_cumulative[[2]][[i]]
      Cross_cov[[1]][[i]] = ginv(Cross_cov[[1]][[i]])
      
      for ( t in 2:(T-2)){
        Cross_cov[[t]][[i]][1:d,1:d] = (trans_sd[i]^(-2))*diag(d)+ ginv(Forward.var.new[[t-1]][[i]]) +(trans_sd[i]^(-2))*diag(d)+V_X_cumulative[[t]][[i]]
        Cross_cov[[t]][[i]][1:d,(d+1):(2*d)] = -(trans_sd[i]^(-2))*diag(d)
        Cross_cov[[t]][[i]][(d+1):(2*d),1:d] = -(trans_sd[i]^(-2))*diag(d)
        Cross_cov[[t]][[i]][(d+1):(2*d),(d+1):(2*d)] = (trans_sd[i]^(-2))*diag(d)+ginv(Backward.var.new[[t+2]][[i]])+(trans_sd[i]^(-2))*diag(d)+V_X_cumulative[[t+1]][[i]]
        Cross_cov[[t]][[i]] = ginv(Cross_cov[[t]][[i]])
      }
      
      Cross_cov[[T-1]][[i]][1:d,1:d] = (trans_sd[i]^(-2))*diag(d)+ ginv(Forward.var.new[[T-2]][[i]]) +(trans_sd[i]^(-2))*diag(d)+V_X_cumulative[[T-1]][[i]]
      Cross_cov[[T-1]][[i]][1:d,(d+1):(2*d)] = -(trans_sd[i]^(-2))*diag(d)
      Cross_cov[[T-1]][[i]][(d+1):(2*d),1:d] = -(trans_sd[i]^(-2))*diag(d)
      Cross_cov[[T-1]][[i]][(d+1):(2*d),(d+1):(2*d)] = (trans_sd[i]^(-2))*diag(d)+V_X_cumulative[[T]][[i]]
    ...
    if(train_auc[k]-train_auc[k-1]< gap && k>min_iter+1){
      ...
      for (i0 in 1:n){
        ...
        b0[i0] = b0[i0] + sum(Mean_X_new[[1]][i0,]^2)+sum(diag(x=Sigma_X_new[[1]][[i0]]))
        
        
        ded_free = 1-(T-1)*d/2
        
        tau_inv_sq = exp(besselK.nuAsym(x=sqrt(b[i0]), nu=-ded_free-1,k.max=5,log=T)-besselK.nuAsym(x=sqrt(b[i0]), nu=-ded_free,k.max=5,log=T))/sqrt(b[i0])-2*ded_free/b[i0]
        trans_sd[i0] = as.numeric(1/sqrt(tau_inv_sq))
      ...
      sg_inve_sq = (n*d+1)/(sum(b0)+1)
      
      sigma_X1 = rep(as.numeric(1/sqrt(sg_inve_sq)),n)
