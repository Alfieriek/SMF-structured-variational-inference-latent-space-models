      ...
      for(sim_id in 1:simulation_count) {
        ...
        for(max_iter in c(50, 100, 200)) {
          ...
          MF_list_adaptive = mcmc_GaussianDN_adaptive(Y=Y, mean_beta_prior=mean_beta_prior, d=d,
                                                      max_iter=max_iter, sigma_beta_prior=sigma_beta_prior, sigma=sigma,  gap=1e-4)
          end_time <- Sys.time()
          ...
          start_time <- Sys.time()
          Mix_list_adaptive = mix_Gaussian_adaptive(Y=Y, mean_beta_prior=mean_beta_prior,d=d,
                                                    global_prior='Cauthy',
                                                    sigma_beta_prior=sigma_beta_prior, sigma=sigma,  gap=1e-4)
          end_time <- Sys.time()
          ...
          error <- sqrt(sum(Mix_adaptive_sm))
          ...
          write.table(results, file = "simulation_results_Gaussian.csv", append = TRUE, sep = ",", col.names = FALSE, row.names = FALSE)
...
write.table(data.frame(
  ...
  time = numeric(),
  error = numeric()
), file = "simulation_results_Gaussian.csv", append = FALSE, sep = ",", col.names = TRUE, row.names = FALSE)
